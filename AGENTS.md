...
**Synthetic self-event on send.** Every successful `sendMessage` / `sendMediaGroup` from the bot immediately synthesizes a `CanonicalMessageEvent` (`source: 'bot'`, `isSelfSent: true`) and injects it into the pipeline. The userbot will deliver the same message back through ingress shortly; Projection's messageId-keyed dedup keeps whichever event arrives first and merges `isSelfSent` onto the existing node. This closes the probe race window: without it, a probe triggered between send-success and userbot-echo would judge the wake-up blind to the bot's just-sent reply (the primary's view doesn't suffer because send_message tool results live in TRs, but the probe deliberately skips the `send_message` tool call when synthesizing tool-call segments — it expects the message to appear as RC).
